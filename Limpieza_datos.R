...
serie_completa <- dbReadTable(con_tesis,"serie_completa")
...
series_trf = series_trf %>%
  mutate(
    fecha = as.Date(paste0(as.character(periodo), '01'), 
                    format = '%Y%m%d')
  ) %>%
  filter(
    kbps >= 1 & 
...
# Paso 1: función para detectar anomalías 
...
series_clean <- series_clean %>% mutate(kbps_corregido = ifelse(kbps_corregido < 0, kbps, kbps_corregido))

# Agrega utilizacion a las series

serie_utilizacion <- dbReadTable(con_tesis,'serie_completa') %>%
...
dbWriteTable(con_tesis, 'serie_limpia', serie_limpia_final, overwrite = T) 
dbWriteTable(con_tesis,'serie_poca_historia',serie_PH_final , overwrite= T)
